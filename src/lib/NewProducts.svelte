    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-2">
        ...
        <div class="mx-4  flex flex-col items-center ">
            ...
            <div class="relative odd:content-['sale']">
                ...
                <button on:click={addToCart(p)} class="absolute text-sm font-extrabold font-sans text-white top-[4rem] px-2 rounded-bl-md left-[4rem] bg-slate-600 hover:bg-slate-500 hover:duration-500 -translate-x-1/2 -translate-y-1/2">
            ADD TO CART</button>
            </div>
            <h2 class="text-2xl font-bold">{p.name}</h2>
            <div class="flex gap-4 items-center justify-between">
            <p class="text-lg font-extrabold text-slate-600">{p.price.toLocaleString('en-US', {
                style: 'currency',
                currency: 'USD'
                })}
            </p>
